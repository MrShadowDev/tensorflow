...
class TFRGen(transformer.CodeGenerator):
  ...
  def visit_Subscript(self, node):
    ...
    if isinstance(s, ast.Constant):
      ...
      self._emit_with_loc(f'\n{idx_val} = arith.constant {s.value} : index', node)
    else:
  ...
  def visit_UnaryOp(self, node):
    ...
    if isinstance(node.op, ast.USub):
      ...
      if ty in [TFRTypes.I32, TFRTypes.I64]:
        self._emit_with_loc(f'\n{zero_value} = arith.constant 0 : {ty}', node)
        self._emit_with_loc(
            f'\n{ssa_value} = arith.subi {zero_value}, {value} : {ty}', node)
      elif ty == TFRTypes.F32:
        self._emit_with_loc(f'\n{zero_value} = arith.constant 0.0 : {ty}', node)
        self._emit_with_loc(
            f'\n{ssa_value} = arith.subf {zero_value}, {value} : {ty}', node)
      else:
